package org._movie_hub._next_gen_edition._server;
...
import org._movie_hub._next_gen_edition._server._response_model.StandardResponse;
import org._movie_hub._next_gen_edition._server._response_model.StatusResponse;
...
public class Services extends Watchdog {
    ...
    public void app_details() {
        get(CONTEXT_PATH.concat("/"), (((request, response) -> {
            ...
            if (jsonObject.size() == 0) {
                ...
                jsonObject.add("shopName", new Gson().toJsonTree("UNKNOWN", String.class));
            }
    ...
    private void provide_list() {
        get(CONTEXT_PATH.concat("/trailer"), ((request, response) -> {
            ...
            response.status(HttpURLConnection.HTTP_OK);
            try {
                final JsonArray jsonArray = get_app_details_as_object(new File(TRAILER_KEY_JSON_FILE));
                return new Gson().toJson(new StandardResponse(StatusResponse.SUCCESS, new Gson().toJsonTree(jsonArray, JsonArray.class)));
            } catch (Exception exception) {
                exception.printStackTrace();
                new Thread(write_stack_trace(exception)).start();
